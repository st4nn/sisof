...
function ready_login()
{
  ...
  **/
  ...
  var Usuario = JSON.parse(localStorage.getItem('ls_sisof'));  
  ...
  if (Usuario != null)
  {
    var objUser = JSON.parse(localStorage.getItem('ls_sisof'));
    var cDate = new Date();
  ...
  {
    objUser.cDate = cDate;
    localStorage.setItem("ls_sisof", JSON.stringify(objUser));    
    window.location.replace("aplicacion/index.html");
  } else
  {
    delete localStorage.ls_sisof;    
  }
}
/**
  ...
  var cDate = new Date();
    ...
    $.post('server/php/proyecto/validarUsuario.php', {pUsuario: pUsuario, pClave : pClave, pFecha : cDate}, function(data, textStatus, xhr) 
    {
      if (data != 0)
      {
        ...
        {
          data.cDate = cDate;
          localStorage.setItem("ls_sisof", JSON.stringify(data));  
